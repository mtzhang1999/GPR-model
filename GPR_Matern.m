close all; clear all; clc;
file_name = 'MSFT-Max.csv';
total_len = 1000;
inverse = 0;
[raw_data, ~] = Data_Preprocess(file_name, total_len, inverse);
len = length(raw_data);
t = (1:len)';
train_len = 965;
validation_len = 25;
data_ave = mean(raw_data(1:train_len));
y = raw_data - data_ave;
test_len = total_len - train_len - validation_len;
train_y = y(1:train_len);
train_t = t(1:train_len);
val_y = y(train_len + 1:train_len + validation_len);
val_t = t(train_len + 1:train_len + validation_len);
%% PSO 
% PSO constant definition
w = 0.6;
c1 = 2;
c2 = 2;
% Particle Matrix Length: 4
% Current Position:1 
% Velocity:1
% Personal Best:1
% Best MSE:1
l_interval = [0, 40];
Global_Best = ones(1, 2) * 3e8;
iter_Max = 50;
MSE_plot = zeros(1, iter_Max);
v_max = 5;
% Random Initialization
Rand_l = rand([50, 1]) * l_interval(2);
Rand_v = rand([50, 1]) * 2;
% Define particles
particles = [Rand_l, Rand_v, ones(50, 2)*3e8];
for iter = 1:iter_Max
    % Calculate MSE in current position
    fprintf("Iteraion %d starts\n", iter);
    MSE = zeros(1, 50);
    for par = 1: 50
        l = particles(par, 1);
        [K_all, ~] = kernel_Matern([train_t; val_t], l);
        K = K_all(1:train_len, 1:train_len);
        K_s = K_all(train_len+1:train_len+validation_len, 1:train_len);
        y_bar = K_s*(K^-1)*train_y;
        MSE(par) = Error_cal(val_y, y_bar); %MSE cal
        if MSE(par) < particles(par, 4) %Update personal best
            particles(par, 3) = l;
        end
        if MSE(par) < Global_Best(2) %Update global best
            Global_Best = [l, MSE(par)];
        end
    end
    MSE_plot(iter) = Global_Best(2);
    fprintf("Current best %f \n", Global_Best(2));
    % Update velocity and position
    for par = 1:50
        v1 = particles(par, 3)-particles(par, 1);
        v2 = Global_Best(1)-particles(par, 1);
        new_v = w*particles(par, 2) + c1*rand()*v1 + c2*rand()*v2;
        new_v = min(new_v, v_max);
        new_v = max(new_v, -v_max);
        particles(par, 2) = new_v;
        particles(par, 1) = particles(par, 1) + new_v;
        particles(par, 1) = Boundary_Check(particles(par, 1), l_interval);
    end
end
plot(MSE_plot)
%% Plot
tr_y = train_y;
l = Global_Best(1);
len_tr = train_len;
len = total_len;
t = (1:len)';
[K_all, dis] = kernel_Matern(t, l);
K = K_all(1:len_tr, 1:len_tr);
K_s = K_all(len_tr+1:len, 1:len_tr);
K_ss = K_all(len_tr+1:len, len_tr+1:len);
y_bar = K_s*(K^-1)*tr_y + data_ave;
var_y = K_ss - K_s/K*(K_s');
str = sprintf("Matern %s, l=%f",file_name, l);
figure;
subplot(1, 2, 1);
hold on;
plot(raw_data, 'b-x');
plot(len_tr+1:len_tr+validation_len, y_bar(1:validation_len), 'g-o');
plot(len_tr+validation_len+1:len, y_bar(validation_len+1:end), 'r-o');
plot(ones(1,length(raw_data))*data_ave, 'b');
MSE = Error_cal(y_bar(validation_len+1:end), raw_data(train_len+validation_len, end));
legend("Original Data", "Validation", "Prediction", "Original Data Average");
xlabel("Trading Day");
ylabel("US Dollar($)");
title(str);
hold off;
subplot(1, 2, 2);
hold on;
plot(len_tr+1:len, raw_data(len_tr+1:len), 'b-x');
plot(len_tr+1:len_tr+validation_len, y_bar(1:validation_len), 'g-o');
plot(len_tr+validation_len+1:len, y_bar(validation_len+1:end), 'r-o');
plot(len_tr+1:len, ones(1,len-train_len)*data_ave, 'b');
MSE = Error_cal(y_bar(validation_len+1:end), raw_data(train_len+validation_len+1: end));
legend("Original Data", "Validation", "Prediction", "Original Data Average");
xlabel("Trading Day");
ylabel("US Dollar($)");
title(str+' Zoomed');
hold off;
disp(MSE);
%% Function
function out = Boundary_Check(pos, l)
    pos = max(min(l(2), pos), l(1));
    out = pos;
end